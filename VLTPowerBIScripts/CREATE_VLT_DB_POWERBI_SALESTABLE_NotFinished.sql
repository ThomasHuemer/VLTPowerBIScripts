...
IF EXISTS (
		SELECT 1
		FROM sys.VIEWS
		WHERE NAME = 'VLT_DB_POWERBI_SALESTABLE_NotFinished'
			AND TYPE = 'V'
	)
	DROP VIEW VLT_DB_POWERBI_SALESTABLE_NotFinished;
GO

CREATE VIEW [dbo].[VLT_DB_POWERBI_SALESTABLE_NotFinished]
AS
	SELECT SALESTABLE.SALESID,
		   ...
		   DATEPART(WEEK, SALESTABLE.DELIVERYDATE) AS DeliveryWeek,
		   (SELECT TOP 1 it.PRODPOOLID FROM SALESLine JOIN inventTable it ON (it.ITEMID = salesLine.ITEMID) WHERE SAlesLine.SalesId = SalesTable.SALESID) AS ProdPool, 
	...
	WHERE SALESTABLE.DATAAREAID = '100'
		...
		--AND SALESTABLE.CREATEDDATETIME > DATETIMEFROMPARTS(YEAR(GETDATE()), 1, 1, 0, 0, 0, 0)
		AND SALESTABLE.DELIVERYDATE > DATETIMEFROMPARTS(YEAR(GETDATE()), 1, 1, 0, 0, 0, 0)
		AND SalesTable.VLT_SALESORDERPRODREGSTATUS != 2
		--AND DATEPART(Year, SALESTABLE.DELIVERYDATE) = DATEPART(Year, GetDate())
